...
export default function DisplayProtfolio() {
    ...
    let i=1;
     ...
     const fatchUsers= async()=>{
      try{
          const res = await axios.get("/protfolio");
          setProtfolios(res.data);                 
      }catch(err){
          console.log(err)
      }            
  }
     ...
     const handleDelete=async(e)=>{
      let text = "Are you sure?";
       ...
       if (window.confirm(text) === true) {
        try{
            const res = await axios.delete("/protfolio/"+e);
            alert(res.data)
...
<main id="main" class="main">
  ...
  <div class='col-lg-12'>
    <div class='card'>
      <div class='card-body'>
        <h5 class='card-title'>Display Protfolio</h5>
        ...
        <div class='dataTable-wrapper dataTable-loading no-footer sortable searchable fixed-columns'>
          ...
          <div class='dataTable-container'>
            <table id="example" class="table table-striped dt-responsive nowrap w-100">
              ...
              <tbody>
              {protfolios.map((protfolio)=>
                       <tr>
                      
                        <td scope="row">{i++}</td>
                         <td>{protfolio.protfolio_name}</td>
                         <td>{protfolio.protfolio_web_url}</td>
                         <td>{protfolio.protfolio_app_url}</td>
                         <td>{protfolio.status}</td>
                                               
                         <td>
                             <button className="delete" onClick={()=>handleDelete(protfolio.protfolio_id)}>Delete</button>
                        &nbsp; &nbsp;
                             <button className="edit"><Link to={`/protfolio/${protfolio.protfolio_id}`}>Edit</Link></button>
                         </td>
